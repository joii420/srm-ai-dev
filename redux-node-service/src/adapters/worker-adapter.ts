...
export class NodeEvalService {
  ...
  /** 预创建的 Worker（来自 WorkerPool），start() 时直接使用而非新建 */
  private _preWarmedWorker: Worker | null = null;

  constructor(preWarmedWorker?: Worker) {
    ...
    this._preWarmedWorker = preWarmedWorker ?? null;
  }
  ...
  *start() {
    ...
    if (this._preWarmedWorker) {
      // 使用预创建的 Worker（已完成模块加载 + linkedom 预加载）
      this._worker = this._preWarmedWorker;
      this._preWarmedWorker = null;
    } else {
    ...
    this._worker.on("exit", (code) => {
      ...
      this._isReady = false;
      this._worker = null;
      // Worker 退出后，通知所有等待中的 request channel，
      // 防止 saga yield take(ch) 永久挂起
      for (const [id, ch] of this._channels) {
        try {
          ch.put({ data: { errors: [{ message: `Worker exited with code ${code}` }] }, startTime: 0, endTime: 0 });
        } catch {}
      }
  ...
  *shutdown() {
  ...
  /**
   * 释放未使用的预热 Worker（如果 start() 从未被调用）。
   * 在 session 创建失败时调用，防止 Worker 线程泄漏。
   */
  dispose() {
    if (this._preWarmedWorker) {
      this._preWarmedWorker.terminate();
      this._preWarmedWorker = null;
    }
  }
