...
import { getTokenStorage } from "ce/utils/tool";
import { getCheckStatus } from "selectors/editorSelectors";
import { PERMISSION_TYPE } from "@appsmith/utils/permissionHelpers";
const forbidPermissions = [
...
export const blockedApiRoutesForAirgapInterceptor = async (
  ...
  config.headers["USER-ENV"] = localStorage.getItem("USER-ENV") ?? "";
...
export const apiSuccessResponseInterceptor = (
  ...
  // const checkStatusInfo = getCheckStatus(store.getState());
  // const checkStatus = checkStatusInfo?.state;
  // const IsSame = checkStatusInfo?.isSame;
  // const readOnly = checkStatus && (checkStatus !== "2" || !IsSame);
  // if (readOnly) {
  //   // forbid permissions for query actions, js actions, datasources
  //   if (
  //     (url.includes("/actions?applicationId") ||
  //       url.includes("/datasources?workspaceId")) &&
  //     response.config.method === "get"
  //   ) {
  //     return {
  //       ...response.data,
  //       data: response.data?.data?.map((a: any) => ({
  //         ...a,
  //         userPermissions: a.userPermissions?.filter(
  //           (p: any) => !forbidPermissions.includes(p),
  //         ),
  //       })),
  //     };
  //   }
  //   if (url.includes('/collections/actions/')) {
  //     return {
  //       ...response.data,
  //       data: {
  //         ...response.data?.data,
  //         userPermissions: response.data?.data.userPermissions?.filter(
  //           (p: any) => !forbidPermissions.includes(p),
  //         ),
  //       },
  //     };
  //   }
  //   // forbid permissions for page, workspace
  //   if (
  //     (url.includes("v1/pages/") ||
  //       (url.includes("v1/workspaces/") && !url.includes("/members"))) &&
  //     response.config.method === "get"
  //   ) {
  //     return {
  //       ...response.data,
  ...
  if (
    ...
    console.error(new Error("Api responded without response meta"), {
  ...
  if (error.response) {
    ...
    if (!is404orAuthPath()) {
      ...
      if (error.response.status === API_STATUS_CODES.REQUEST_NOT_AUTHORISED) {
        ...
        console.error("[api]", error?.message || error?.code || "API error");
        return Promise.reject({
          ...error,
    ...
    // Node.js: only log URL, not the full ClientRequest/Writable object
    const url = error.config?.url || error.request?.path || "unknown";
    log.error(`[api] No response received: ${error.message} → ${url}`);
  } else {
