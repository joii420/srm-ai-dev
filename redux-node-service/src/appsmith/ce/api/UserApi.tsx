...
export class UserApi extends Api {
  ...
  static async grantUserEnv() {
    const userEnv = localStorage.getItem("USER-ENV") as string;
    if (!userEnv || userEnv === "") {
      const url = STEP_SERVER + "/grant/env";
      await fetch(url, {
        method: "GET",
      })
        .then((response) => response.json())
        .then((data) => {
          localStorage.setItem("USER-ENV", data.data);
          return data;
        });
    }
  }

  static async grantUser() {
    const { us } = getPathParams();
    // const setToken = (k: string, token: string) => {
    //   // const key = APP_STORE_NAMESPACE + "-" + k;
    //   // const cache = localStorage.getItem(key) ?? "{}";
    //   // const result = Object.assign({}, JSON.parse(cache), {
    //   //   [us]: token,
    //   // });
    //   // localStorage.setItem(key, JSON.stringify(result));
    //   // localStorage.setItem(us, token);
    //   setTokenStorage(token);
    // };
    if (us && !isUsStorageExist(us)) {
      const url = GRANT_SERVER + us;
      ...
      const { expiredAt, token } = data;
      setTokenStorage(token, expiredAt);
      // if (typeof applicationIds === "string") {
      //   setToken(applicationIds, token);
      // } else if (isArray(applicationIds)) {
      //   applicationIds.forEach((i) => setToken(i, token));
      // }
    }
    // fetchUserKeys(REMOVEKEY_SERVER);
    return "success";
  }
