...
import { fetchAllPackagesOfWorkspaceSaga } from "./PackageSagas";
import {
  getModulesMetadata,
  getPackagesList,
...
export function* createModuleInstanceSaga(
  ...
  try {
    ...
    if (name === "MODULE_INSTANCE_NAME_EXISTS") {
      toast.show("模块名称已存在!", {
        kind: "error",
      });
      return;
...
function* fetchModuleInstancesSaga(
  ...
  try {
    ...
    const showModuleReference: boolean = false;
...
export function* setupModuleInstanceSaga(
  ...
  try {
    ...
    if (!packagesList.length) {
      yield fork(fetchAllPackagesOfWorkspaceSaga, {
...
export function* setupModuleInstanceForViewSaga(
  ...
  try {
    ...
    const instanceFetchSuccess: boolean = yield failFastApiCalls(
      [
        fetchModuleInstances({ contextId, contextType, viewMode: false }),
        fetchModuleInstanceEntities({
          contextId,
          contextType,
          viewMode: false,
