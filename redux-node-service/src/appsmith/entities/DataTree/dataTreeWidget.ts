...
const generateDefaultMetaProps = (
  widget: FlattenedWidgetProps,
): {
  defaultMetaProps: Record<string, unknown>;
} => {
...
const generateDefaultMetaPropsMemoized = memoize(generateDefaultMetaProps, {
  maxSize: 1000,
});
...
export const generateDataTreeWidgetReturnMeta = (
  widget: FlattenedWidgetProps,
  widgetMetaProps: Record<string, unknown> = {},
  loadingEntities: LoadingEntitiesState,
  layoutSystemType: LayoutSystemTypes = LayoutSystemTypes.FIXED,
  ...
  // overridingMetaPropsMap,
  // } = generateDataTreeWidgetWithoutMetaMemoized(widget);
  const { defaultMetaProps } = generateDefaultMetaPropsMemoized(widget);
  // const overridingMetaProps: Record<string, unknown> = {};
  return { defaultMetaProps: defaultMetaProps };
