...
// Node.js: navigator.geolocation not available, return empty coords
export const getUserLocation = async (_options?: PositionOptions) =>
  new Promise((resolve) => {
    console.warn("[geolocation] Geolocation not available in Node.js, returning empty coords");
    resolve({
      coords: {
...
export function* watchCurrentLocation(
  ...
  // Node.js: geolocation.watchPosition not available
  console.warn("[geolocation] watchPosition not available in Node.js");
  watchId = -1 as any;
}
...
export function* stopWatchCurrentLocation() {
  ...
  // Node.js: geolocation.clearWatch not available
  watchId = undefined;
