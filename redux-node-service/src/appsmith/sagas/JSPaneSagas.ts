...
import {
  ...
  getRefactorJSCollectionActionApis,
  updateJSCollectionApis,
...
function* handleEachUpdateJSCollection(update: JSUpdate) {
  ...
  try {
      ...
      if (parsedBody && !!jsAction) {
        ...
        if (data.nameChangedActions.length) {
          for (let i = 0; i < data.nameChangedActions.length; i++) {
            const pageId = data.nameChangedActions[i].pageId || "";
            // TODO: 【临时修改】调整逻辑，不调用接口，改为返回调用接口
            // yield put(
            //   refactorJSCollectionAction({
            //     refactorAction: {
            ...
            const httpAction = getRefactorJSCollectionActionApis({
              refactorAction: {
                actionId: data.nameChangedActions[i].id,
                collectionName: jsAction.name,
                pageId: data.nameChangedActions[i].pageId || "",
          ...
          if (updateCollection) {
            ...
            // });
            const httpAction = updateJSCollectionApis({
              jsCollection: jsActionTobeUpdated,
              newActions: newActions,
              updatedActions: updateActions,
              deletedActions: deletedActions,
    ...
    yield put({ type: ReduxActionTypes.NODE_UPDATE_JS_BODY_SUCCESS, payload: httpActions });
  } catch (error) {
    yield put({type: ReduxActionTypes.NODE_UPDATE_JS_BODY_ERROR, payload: error });
    throw error;
  }
}
...
export function* handleStartExecuteJSFunctionSaga(
  ...
  const actionId = action.id;
  const state: AppState = yield select((state: AppState) => state);
  const JSActionPathName = getJSActionPathNameToDisplay(action, collection, state);
