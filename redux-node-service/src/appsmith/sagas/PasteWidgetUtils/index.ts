...
const getNewPositions = function* (
  ...
  //if widget type is sftab、dashboard and so on,get the selectedtabid
  let selectedTabId;
  const selectedWidgetInfo = selectedWidgets[0];
  if (
    selectedWidgetInfo &&
    (selectedWidgetInfo.type === "SFTAB_WIDGET" ||
      selectedWidgetInfo.type === "SFDASHBOARD_WIDGET")
  ) {
    const selectedTabWidgetId: string = yield select(
      getSFTabSelectedTabWidgetId,
      selectedWidgetInfo,
    );
    selectedTabId = selectedTabWidgetId;
  }
  ...
  if (
    ...
    (copiedWidgetGroups[0].list[0].type === "MODAL_WIDGET" ||
      copiedWidgetGroups[0].list[0].type === "DRAWER_WIDGET")
  )
    return {};
...
function* getNewPositionsBasedOnMousePositions(
  ...
  } = getDefaultCanvas(canvasWidgets, ps);
  ...
  if (pastingIntoWidgetId && pastingIntoWidgetId !== MAIN_CONTAINER_WIDGET_ID) {
    ...
    const containerWidgetId = getContainerIdForCanvas(pastingIntoWidgetId, ps);
...
function* getNewPositionsBasedOnSelectedWidgets(
  ...
  const containerId = getContainerIdForCanvas(parentId, ps);
  ...
  const canvasDOM = queryPageSelector(`#${getSlidingArenaName(parentId)}`, ps);
