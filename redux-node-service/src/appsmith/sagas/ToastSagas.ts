import type { ToastProps } from "../stubs/toast";
...
export default function* showToast(
  ...
  let debugFlag: string | null = null;
  try {
    const urlObject = new URL(window?.location?.href);
    debugFlag = urlObject?.searchParams?.get("debug");
  } catch {
    // Node.js: window.location.href may be empty
  }
