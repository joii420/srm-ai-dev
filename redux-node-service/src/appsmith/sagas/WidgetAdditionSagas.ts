...
function* addNewTabChildSaga(
  ...
  const { widgetId, controlType } = addChildTabAction.payload;
  const tabProps: WidgetProps = yield select(getWidget, widgetId);
  let splitObj = tabProps.splitterObj;
  let panelsObj = tabProps.panelsObj;
  let tabs = tabProps.tabsObj;
  let holidayObj = tabProps.holidayObj;
  let eventMarkerObj = tabProps.eventMarkerObj;
  if (controlType === "HOLIDAY_CONTROL" && holidayObj) {
    const holidayArray = Object.values(holidayObj);
    const newHolidayWidgetId = generateReactKey() as any;
    const newHolidayId = holidayArray.length + 1 + "";
    const newHolidayLabel = getNextEntityName(
      "Holiday ",
      holidayArray.map((holiday: any) => holiday.label),
    );
    holidayObj = {
      ...holidayObj,
      [newHolidayId]: {
        id: newHolidayId,
        index: holidayArray.length,
        label: newHolidayLabel,
        widgetId: newHolidayWidgetId,
        isVisible: true,
      },
    };
    const newHolidayProps: any = getChildTabData(tabProps, {
      id: newHolidayId,
      label: newHolidayLabel,
      widgetId: newHolidayWidgetId,
    });
    ...
    updatedWidgets[widgetId]["holidayObj"] = holidayObj;
    yield put(updateAndSaveLayout(updatedWidgets));
  }

  // 处理新增splitter分割块
  if (splitObj) {
    const splitArray = Object.values(splitObj);
    const newSplitWidgetId = generateReactKey() as any;
    // const newSplitId = generateReactKey({ prefix: "Split" });
    const newSplitId = splitArray.length + 1 + "";
    const newSplitLabel = getNextEntityName(
      "Split ",
      splitArray.map((split: any) => split.label),
    );
    splitObj = {
      ...splitObj,
      [newSplitId]: {
        ...
        label: newSplitLabel,
        widgetId: newSplitWidgetId,
        isVisible: true,
        size: "10%",
      },
    };
    const newSplitProps: any = getChildTabData(tabProps, {
      id: newSplitId,
      label: newSplitLabel,
      widgetId: newSplitWidgetId,
    });
    ...
    updatedWidgets[widgetId]["splitterObj"] = splitObj;
    yield put(updateAndSaveLayout(updatedWidgets));
  }
  if (panelsObj) {
    const panels = Object.values(panelsObj);
    const newPanelWidgetId = generateReactKey() as any;
    // const newSplitId = generateReactKey({ prefix: "Split" });
    const newPanelId = `Panel${panels.length + 1}`;
    const newPanelLabel = getNextEntityName(
      "Panel ",
      panels.map((p: any) => p.label),
    );
    panelsObj = {
      ...panelsObj,
      [newPanelId]: {
        ...
        index: panels.length,
        label: newPanelLabel,
        widgetId: newPanelWidgetId,
        sizeX: 1,
        sizeY: 1,
        enabled: true,
      },
    };
    const newpanelProps: any = getChildTabData(tabProps, {
      id: newPanelId,
      label: newPanelLabel,
      widgetId: newPanelWidgetId,
    });
    ...
    updatedWidgets[widgetId]["panelsObj"] = panelsObj;
    yield put(updateAndSaveLayout(updatedWidgets));
  }
  if (tabs) {
  ...
  //甘特图事件标记添加
  if (controlType === "EVENT_MARKER_CONTROL" && eventMarkerObj) {
    const eventMarkerArray = Object.values(eventMarkerObj);
    const newEventMarkerWidgetId = generateReactKey() as any;
    const newEventMarkerId = generateReactKey({ prefix: "eventMarker" });
    const newEventMarkerLabel = getNextEntityName(
      "Marker ",
      eventMarkerArray.map((marker: any) => marker.label),
    );
    ...
    eventMarkerObj = {
      ...
      [newEventMarkerId]: {
        id: newEventMarkerId,
        index: eventMarkerArray.length,
        label: newEventMarkerLabel,
        widgetId: newEventMarkerWidgetId,
        isVisible: true,
      },
    };
    const newEventMarkerProps: any = getChildTabData(tabProps, {
      id: newEventMarkerId,
      label: newEventMarkerLabel,
      widgetId: newEventMarkerWidgetId,
    });
    ...
    updatedWidgets[widgetId]["eventMarkerObj"] = eventMarkerObj;
    yield put(updateAndSaveLayout(updatedWidgets));
  }
}
