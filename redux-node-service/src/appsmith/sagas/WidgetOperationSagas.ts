...
import { updateDslSaga } from "ce/sagas/updateDslSaga";
...
export function* buttonSaveAndSetTimeSave(
  action: ReduxAction<BatchUpdateWidgetDynamicPropertyPayload>,
) {
  const stateWidgets: CanvasWidgetsReduxState = yield select(getWidgets);
  // Save the layout
  yield put(updateAndSaveLayout(stateWidgets, {}, true));
}
...
export function getPropertiesToUpdate(
  ...
  Object.keys(updatePaths).forEach((propertyPath) => {
    ...
    if (
      ...
      // &&
      // !propertyPath.includes("primaryColumns")
    ) {
      return;
    }
...
function* pasteWidgetSaga(action: ReduxAction<PasteWidgetReduxAction>) {
  ...
  try {
    ...
    // Node.js: flashElementsById is a DOM operation, skip in server context
    if (typeof document !== "undefined" && document.getElementById && (shouldGroup || !newPastingPositionMap)) {
...
export default function* widgetOperationSagas() {
  ...
  yield all([
    ...
    //新加方法
    takeEvery(
      ReduxActionTypes.BUTTON_SAVE_AND_SET_TIME_SAVE,
      buttonSaveAndSetTimeSave,
    ),
    takeEvery(ReduxActionTypes.GENERATE_UI_BY_DSL, updateDslSaga),
    takeEvery(
