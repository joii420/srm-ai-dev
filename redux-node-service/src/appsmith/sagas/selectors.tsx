...
export const getSFTabSelectedTabWidgetId = createSelector(
  [getWidgetsMeta, (_state: AppState, widget: WidgetProps) => widget],
  (metaState, widget: WidgetProps) => {
    let selectedTabWidgetId =
      metaState[widget.metaWidgetId || widget.widgetId]?.selectedTabWidgetId;
    if (!selectedTabWidgetId)
      selectedTabWidgetId = widget.children?.[widget.deaultSelectedItem];
...
export const getParentModalWidget = (
  state: AppState,
  widgetId: string,
): DataTreeEntity | undefined => {
  const widgets = getWidgets(state);
  const dataTrees = getDataTree(state);
  let modalWidget = undefined;
  let parentId = widgets[widgetId]?.parentId;
  let parentWidget = parentId ? widgets[parentId] : undefined;

  while (parentWidget) {
    if (
      parentWidget.type === "MODAL_WIDGET" ||
      parentWidget.type === "DRAWER_WIDGET"
    ) {
      return dataTrees[parentWidget.widgetName];
    }
    parentWidget = parentWidget.parentId
      ? widgets[parentWidget.parentId]
      : undefined;
  }
