...
export const getDrawerWidgetType = createSelector(
  getIsAnvilLayout,
  (isAnvilLayout: boolean) => {
    const modalWidgetType = "DRAWER_WIDGET";
    return modalWidgetType;
  },
);

export const getDrawerWidgets = createSelector(
  getCanvasWidgets,
  getDrawerWidgetType,
  (widgets, modalWidgetType) => {
    const drawerWidgets = Object.values(widgets).filter(
      (widget: FlattenedWidgetProps) => widget.type === modalWidgetType,
    );
    if (drawerWidgets.length === 0) return undefined;
    return drawerWidgets;
  },
);

export const getDrawerDropdownList = createSelector(
  getDrawerWidgets,
  (drawerWidgets) => {
    if (!drawerWidgets) return undefined;

    return drawerWidgets.map((widget: FlattenedWidgetProps) => ({
      id: widget.widgetId,
      label: widget.widgetName,
      value: `${widget.widgetName}.name`,
    }));
...
export const getNextDrawerName = createSelector(
  getExistingWidgetNames,
  getDrawerWidgetType,
  (names, modalWidgetType) => {
    const prefix =
