...
export const getDifferenceInJSCollection = (
  ...
  if (toBearchivedActions.length && newActions.length) {
    const newActionsCopy = newActions.slice();
    for (let i = 0; i < newActionsCopy.length; i++) {
      const nameChange = toBearchivedActions.find(
        (js) => js.actionConfiguration.body === newActionsCopy[i].body,
      );
      if (nameChange) {
...
export const getRefactorJSCollectionActionApis = (data: any) => {
  const { actionCollection, refactorAction, layoutId } = data;
  const requestData = {
    ...refactorAction,
    layoutId,
  ...
  const httpAction = {
    method: 'PUT',
    url: JSActionAPI.url + "/refactorAction",
    body: requestData,
  };
  return httpAction;
}

export const updateJSCollectionApis = (data: any) => {
  const { jsCollection } = data;
  const httpAction = {
    method: 'PUT',
    url: `${JSActionAPI.url}/${jsCollection.id}`,
    body: jsCollection,
  };
  return httpAction;
}
