...
import { getPageElemById, queryPageSelector } from "utils/hooks/useDomHelper";
...
export function directlyMutateDOMNodes(
  ...
  for (const widgetId in updates) {
    ...
    let widgetNode = getPageElemById(idSelector, ps);
    if (
    ...
    const widgetBoundary = queryPageSelector(
