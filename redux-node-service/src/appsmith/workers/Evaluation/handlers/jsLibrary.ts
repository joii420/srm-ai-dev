...
import makeJsLibraryDefs from "./makeJsLibraryDefs";
...
export async function loadLibraries(
  request: EvalWorkerASyncRequest<(JSLibrary & { makeDefs?: boolean })[]>,
) {
  resetJSLibraries();
  ...
  try {
    for (const lib of libs) {
      ...
      try {
        ...
        for (let i = 0; i < defaultAccessors.length; i++) {
          ...
          if (makeDefs) {
            defs[accessors[i]] = makeJsLibraryDefs(self[accessors[i]]);
          }
        }
    ...
    (libs || []).forEach((lib) => {
      ...
      JSLibraries.push(lib);
    });
    // JSLibraries.push(...libs);
    JSLibraryAccessor.regenerateSet();
    return { success: true, message, defsMap };
  } catch (e) {
    message = (e as Error).message;
    return { success: false, message, defsMap };
  }
}
