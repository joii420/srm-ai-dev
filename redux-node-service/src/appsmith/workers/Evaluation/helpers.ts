...
import { get, isObject, set, omit } from "lodash";
...
//修改：速度优化
export const getNewDataTreeUpdatesFilterNoNeed = (
  paths: string[],
  dataTree: object,
  oldDataTree: object,
) =>
  paths
    .filter((p) => {
      const segmentedPath = p.split(".");
      return get(dataTree, segmentedPath) !== get(oldDataTree, segmentedPath);
    })
    .map((path) => {
      const segmentedPath = path.split(".");
      get(dataTree, segmentedPath);
      return {
        kind: "N",
        path: segmentedPath,
        rhs: get(dataTree, segmentedPath),
...
// 裁剪 key
// const CUT_KEYS = ["Log", "SQL", "INIT", "VALID", "PROCESS", "COMMON"];
...
const generateDiffUpdates = (
  ...
  //修改
  if (
    dataTree.appsmith.mode === "PUBLISHED" &&
    oldDataTree?.INIT?.g_global?.g_account
  ) {
    for (let key in oldDataTree) {
      if (
        oldDataTree[key].ENTITY_TYPE === "JSACTION" ||
        oldDataTree[key].ENTITY_TYPE === "ACTION"
      ) {
        delete oldDataTree[key];
      }
    }

    for (let key in dataTree) {
      if (
        dataTree[key].ENTITY_TYPE === "JSACTION" ||
        dataTree[key].ENTITY_TYPE === "ACTION"
      ) {
        delete dataTree[key];
      }
    }
  ...
  const updates =
    diff(oldData, newData, (path, key) => {
      // diff(omit(oldDataTree, cutKeys), omit(dataTree, cutKeys), (path, key) => {
...
export const generateSerialisedUpdates = (
  ...
  try {
    ...
    //修改
    return { serialisedUpdates: removedLhs };
  } catch (error) {
    return {
      //修改
      // serialisedUpdates: "[]",
