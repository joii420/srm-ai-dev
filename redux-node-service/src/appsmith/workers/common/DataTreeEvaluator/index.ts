...
//新增变量
let initFieldSource: object = {};
let noNeedDiff: boolean = false;
export default class DataTreeEvaluator {
  ...
  setupFirstTree(
    ...
    for (const key in localUnEvalTree) {
      if (
        localUnEvalTree[key]?.ENTITY_TYPE === "WIDGET" &&
        localUnEvalTree[key]?.type === "SFTREEGRID_WIDGET"
      ) {
        let result = localUnEvalTree[key]["fieldSource"];
        if (typeof result === "string") {
          result = JSON.parse(result);
        }
        set(initFieldSource, key + ".fieldSource", result);
      }
    }
    ...
    if (unEvalTree.appsmith.mode === "PUBLISHED" && dataObj && dataObj.status) {
      noNeedDiff = true;
      //构建树
      createDependencyMapNodes(this, localUnEvalTree, configTree);
      const insDependencies = this.dependencyMap.transformMap(
        dataObj.setFirstDataMap.inDependencies,
      );
      const insInverseDependencies = this.dependencyMap.transformMap(
        dataObj.setFirstDataMap.inInverseDependencies,
      );
      const invalidDependencies = this.dependencyMap.transformMap(
        dataObj.setFirstDataMap.inInvalidDependencies,
      );

      const inverseInvalidDependencies = this.dependencyMap.transformMap(
        dataObj.setFirstDataMap.inInverseInvalidDependencies,
      );
      this.dependencyMap.setDependencies(insDependencies);
      this.dependencyMap.setInvalidDependencies(invalidDependencies);
      this.dependencyMap.setDependenciesInverse(insInverseDependencies);
      this.dependencyMap.setInvalidDependenciesInverse(
        inverseInvalidDependencies,
      );

      this.dependencies = this.dependencyMap.dependencies;
      this.inverseDependencies = this.dependencyMap.inverseDependencies;
    } else {
      // dependencies 需要静态化 第一次需要初始化
    ...
    if (dataObj && !dataObj.status) {
      dataObj.setFirstDataMap.inDependencies = this.dependencyMap.dependencies;
      dataObj.setFirstDataMap.inInverseDependencies =
        this.dependencyMap.inverseDependencies;
      dataObj.setFirstDataMap.inInvalidDependencies =
        this.dependencyMap.invalidDependencies;
      dataObj.setFirstDataMap.inInverseInvalidDependencies =
        this.dependencyMap.inverseInvalidDependencies;
    }
    // this.sortedDependencies = this.sortDependencies(dependMap);
  ...
  evalAndValidateFirstTree(dataObj?: any): {
    evalTree: DataTree;
    evalMetaUpdates: EvalMetaUpdates;
    staleMetaIds: string[];
  } {
    console.log(
      "aync !!!!!!!!!!!! startAppEngine ~ evalFirstTree end----",
      performance.now(),
      Date.now(),
    );
  ...
  setupUpdateTree(
    ...
    //修改configTree 在正式环境中 将jsAction Action移除
    if (unEvalTree.appsmith.mode === "PUBLISHED") {
      for (let key in configTree) {
        if (
          key.endsWith("GLOBAL") ||
          key.endsWith("INIT") ||
          key.endsWith("FEATURE")
        ) {
          continue;
        }
        if (
          (configTree[key].ENTITY_TYPE === "JSACTION" ||
            configTree[key].ENTITY_TYPE === "ACTION") &&
          !configTree[key].moduleInstanceId
        ) {
          delete configTree[key];
        }
      }

      for (let key in unEvalTree) {
        if (
          key.endsWith("GLOBAL") ||
          key.endsWith("INIT") ||
          key.endsWith("FEATURE")
        ...
        if (
          unEvalTree[key].ENTITY_TYPE === "JSACTION" ||
          unEvalTree[key].ENTITY_TYPE === "ACTION"
        ) {
          delete unEvalTree[key];
        }
      }
    }
    this.setConfigTree(configTree);
    ...
    let oldUnEvalTreeJSCollections = getJSEntities(this.oldUnEvalTree);
    let localUnEvalTreeJSCollection = getJSEntities(localUnEvalTree);
    //修改
    if (unEvalTree.appsmith.mode === "PUBLISHED") {
      for (let key in oldUnEvalTreeJSCollections) {
        if (key === "COMMON" || key === "PROCESS" || key === "VALID") {
          delete oldUnEvalTreeJSCollections[key];
        }
      }
      for (let key in localUnEvalTreeJSCollection) {
        if (key === "COMMON" || key === "PROCESS" || key === "VALID") {
          delete localUnEvalTreeJSCollection[key];
        }
      }
    }
    let jsDifferences: Diff<
      Record<string, JSActionEntity>,
      Record<string, JSActionEntity>
    >[] = [];
    if (!noNeedDiff) {
      jsDifferences = profileFn(
    ...
    //修改：速度优化
    if (unEvalTree.appsmith.mode === "PUBLISHED") {
      for (let key in oldUnEvalTreeWithStringifiedJSFunctions) {
        if (
          key.endsWith("GLOBAL") ||
          key.endsWith("INIT") ||
          key.endsWith("FEATURE")
        ...
        if (
          oldUnEvalTreeWithStringifiedJSFunctions[key].ENTITY_TYPE ===
            "JSACTION" ||
          oldUnEvalTreeWithStringifiedJSFunctions[key].ENTITY_TYPE === "ACTION"
        ) {
          delete oldUnEvalTreeWithStringifiedJSFunctions[key];
        }
      }

      for (let key in localUnEvalTreeWithStringifiedJSFunctions) {
        if (
          key.endsWith("GLOBAL") ||
          key.endsWith("INIT") ||
          key.endsWith("FEATURE")
        ...
        if (
          localUnEvalTreeWithStringifiedJSFunctions[key].ENTITY_TYPE ===
            "JSACTION" ||
          localUnEvalTreeWithStringifiedJSFunctions[key].ENTITY_TYPE ===
            "ACTION"
        ) {
          delete localUnEvalTreeWithStringifiedJSFunctions[key];
        }
      }
    }
  ...
  getValueByPath(obj: object, path: string) {
    try {
  ...
  evaluateTree(
    ...
    try {
      for (const fullPropertyPath of evaluationOrder) {
        ...
        if (
          fullPropertyPath.endsWith(".fieldSource") &&
          this.getValueByPath(initFieldSource, fullPropertyPath)
        ) {
          // if (
          //   typeof this.getValueByPath(contextTree, fullPropertyPath) ===
          //   "string"
          // ) {
          set(
            contextTree,
            fullPropertyPath,
            get(initFieldSource, fullPropertyPath),
          );
          set(
            safeTree,
            fullPropertyPath,
        ...
        if (requiresEval) {
          ...
          try {
            //修改：速度优化，在开发模式下先取接口的页面数据，没取到就生成
            if (
              unEvalTree.appsmith.mode === "PUBLISHED" &&
              dataObj &&
              dataObj.status &&
              !fullPropertyPath.includes(".fieldSource")
            ) {
              let data = dataObj.pageData;
              const resultData = this.getValueByPath(data, fullPropertyPath);
              evalPropertyValue = resultData;
            } else if (!dataObj && tempObj[unEvalPropertyValue]) {
              //修改
              evalPropertyValue = tempObj[unEvalPropertyValue];
            } else {
              ...
              //   console.log("www===fullProp", fullPropertyPath);
              // }
              if (!dataObj) {
                if (unEvalPropertyValue.startsWith("{{")) {
                  //修改
                  tempObj[unEvalPropertyValue] = evalPropertyValue;
                }
              }
            }
          } catch (error) {
        ...
        switch (entityType) {
          case ENTITY_TYPE.WIDGET: {
            ...
            // if (fullPropertyPath.includes(".fieldSource")) {
            //   console.log("tttttttttttttttt parsefield", parsedValue);
            //   set(initFieldSource, fullPropertyPath, parsedValue);
            // }
          ...
          case ENTITY_TYPE.JSACTION: {
            ...
            if (!skipVariableValueAssignment) {
              const valueForSafeTree = evalValue;
      ...
      if (dataObj && !dataObj.status) {
        //准备数据初始化
        dataObj.contextTree = contextTree;
        // fetch(
        //   "/api/v1/consolidated-api/savePageConfigData?pageId=" +
        //     dataObj.pageId,
        //   {
        //     method: "POST",
        //     headers: {
        ...
        //       // Authorization: token,
        //     },
        //     body: JSON.stringify(contextTree),
        //   },
        // );
      }
    ...
    if (stringSegments.length) {
      ...
      const values = jsSnippets.map((jsSnippet, index) => {
        ...
        if (jsSnippet) {
          ...
          if (needDealScript === undefined) {
            const { errors: evalErrors, result } =
              this.evaluateDynamicBoundValue(
                toBeSentForEval,
                data,
