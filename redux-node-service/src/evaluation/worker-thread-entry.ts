...
// Diagnostic log — shared fs/path for all handlers below
...
// Catch unhandled errors in worker thread — write to diag file + notify main thread
process.on("uncaughtException", (err: any) => {
  const msg = `[worker] Uncaught: ${err?.message || err}\n${err?.stack?.split("\n").slice(0, 5).join("\n") || ""}\n`;
  try { _diagFs.appendFileSync(_diagPath, msg); } catch {}
