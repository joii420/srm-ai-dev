...
import { deployPageConfig } from "ce/utils/tool";
...
export function* publishApplicationSaga(
  ...
  try {
    ...
    const acct: any = yield select(getCurrentUser);
    ...
    if (isValidResponse) {
      ...
      const isSuccess: boolean = yield deployPageConfig(
        currentPageId,
        currentPageName,
        !!request.ignoreLayout,
        acct?.email,
      );
      if (!isSuccess) {
        toast.show("部署失败", { kind: "error" });
        console.error("部署失败");
      }
      yield put({
  ...
  GlobalMessageHelper.closeByContext("deploy");
}
...
function* updateSocketOpened() {
  yield put({
    type: ReduxActionTypes.SOCKET_CONNECTED,
    payload: true,
  });
}
// v1
export function* fetchAppAndPagesSaga(
  ...
  try {
    ...
    if (isValidResponse) {
      ...
      const data = yield select(getHomepageTextConfig);
      const code = data?.site ?? "";
      const name = data?.applicationTitle ?? "";
      yield put({
        type: ReduxActionTypes.FETCH_APPLICATION_SUCCESS,
        payload: {
          ...response.data.application,
          isModelWorkspace: response.data.isModelWorkspace,
