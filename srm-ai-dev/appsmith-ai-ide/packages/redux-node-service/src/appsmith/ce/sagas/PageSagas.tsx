...
import { getCurrentPageInfo, getEvalMode } from "selectors/onboardingSelectors";
...
import { changeEditLockStatus, getEditLockStatus } from "ce/utils/tool";

const PAGE_MANAGE_PERMISSIONS: any = {
  [PERMISSION_TYPE.MANAGE_PAGES]: true,
  [PERMISSION_TYPE.DELETE_PAGES]: true,
  [PERMISSION_TYPE.CREATE_ACTIONS]: true,
};
...
export function* handleFetchedPage({
  ...
  let pagePermissions = fetchPageResponse.data.userPermissions;
  // 签入签出功能
  let params = {
    acct: currentUser.email,
    code: pageSlug,
  };

  // TODO: 【临时修改】不使用签入签出，默认签出
  // const checkStatus = yield call(getEditLockStatus, params) as any;
  const checkStatus = {
    button: '签入',
    canEdit: true,
    icon: '',
  ...
  // 保存当前页签入签出信息
  yield put({
    type: ReduxActionTypes.SET_CHECK_STATUS,
    payload: { ...checkStatus },
  });
...
export function* savePageSaga(action: ReduxAction<{ isRetry?: boolean }>) {
  ...
  try {
    ...
    if (error instanceof IncorrectBindingError) {
      ...
      if (isRetry) {
        console.error(new Error("Failed to correct binding paths"));
        yield put({
...
export function* saveLayoutSaga(
  action: ReduxAction<{ isRetry?: boolean; isRequest?: boolean }>,
) {
  try {
...
export function* createNewPageFromEntity(
  ...
  try {
    ...
    const { applicationId, name, isModal } = createPageAction?.payload || {};
...
export function* createPageSaga(
  ...
  try {
    ...
    if (isValidResponse) {
      ...
      // 自动签出新页面
      const currentPage: Page = yield select(getPageById(response.data.id));
      const currentUser: User = yield select(getCurrentUser);
      let data = {
        code: currentPage?.pageName,
        acct: currentUser?.email,
        type: "checkOut",
      };
      yield call(changeEditLockStatus, data);
...
export function* populatePageDSLsSaga(action?: {
  ...
  try {
    ...
    // 更新页面列表权限，获取页面列表和页面详情后就可以确认权限，不需要在这里重复设置每个页面详情返回的权限
    // yield put({
