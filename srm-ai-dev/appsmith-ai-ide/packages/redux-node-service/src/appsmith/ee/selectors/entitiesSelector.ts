...
import {
  ...
  getAllModules,
  getAllModulesInEdit,
  getCurrentModule as getCurrentModule2,
...
// duplicated selector
export const getCurrentModule = getCurrentModule2;

export const getInputsForModule = (state: AppState): any => {
  const moduleId = getCurrentModuleId(state);
  const module = state.ui.packages.currentPackage?.modules.find(
...
export const getNewEntityName = createSelector(
  ...
  (actions, jsCollections, moduleInstances = {}, options) => {
    ...
    if (isModuleInstance) {
      if (prefixExists) {
        return "MODULE_INSTANCE_NAME_EXISTS";
      }
      return prefix;
    }
