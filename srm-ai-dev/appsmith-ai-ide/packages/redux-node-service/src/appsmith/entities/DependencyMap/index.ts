...
export default class DependencyMap {
  ...
  public setDependencies(dependencies: TDependencies) {
    this.#dependencies = dependencies;
  }

  public setInvalidDependencies(invalidDependencies: TDependencies) {
    this.#invalidDependencies = invalidDependencies;
  }

  public setDependenciesInverse(dependenciesInverse: TDependencies) {
    this.#dependenciesInverse = dependenciesInverse;
  }

  public setInvalidDependenciesInverse(
    invalidDependenciesInverse: TDependencies,
  ) {
    this.#invalidDependenciesInverse = invalidDependenciesInverse;
  }

  public transformMap(data: object) {
    let deps: Map<string, Set<string>> = new Map();
    for (const key in data) {
      // console.log(
      //   "2-nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnfirst",
      //   key,
      // );
      try {
        if (data[key]) {
          deps.set(key, new Set(data[key]));
          continue;
        }
        deps.set(key, new Set());
      } catch (e) {
        console.log(
          "nnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnnfirst error",
          e,
        );
      }
    }
  ...
  get getDependenciesInverse() {
    return this.#dependenciesInverse;
  }
  get getInvalidDependenciesInverse() {
