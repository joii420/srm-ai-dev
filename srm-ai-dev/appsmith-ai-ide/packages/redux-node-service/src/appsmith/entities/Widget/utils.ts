import PrimaryObjectControl from "components/propertyControls/PrimaryObjectControl";
import type {
  PropertyPaneConfig,
  ValidationConfig,
...
const childHasPanelConfig = (
  ...
  let widgetPanelPropertyValues = get(widget, panelPropertyPath);
  /**
   * 源码修改: 新增对象可视化配置, 由于appsmith配置子面板属性的时候, 默认为多个属性配置, 需要针对特定组件添加额外的规则;
   * 针对PrimaryObjectControl配置, 使用targetPropertyName指定需要更新的属性名称, 更新到对应的对象上;
   */
  const isPrimaryObject =
    config.controlType === PrimaryObjectControl.getControlType();
  if (isPrimaryObject) {
    widgetPanelPropertyValues = {};
    const targetValue = get(
      widget,
      `${panelPropertyPath}.${config.targetPropertyName}`,
    );
    if (targetValue)
      widgetPanelPropertyValues[config.targetPropertyName] = targetValue;
  }
  ...
  if (widgetPanelPropertyValues) {
    Object.values(widgetPanelPropertyValues).forEach(
      (widgetPanelPropertyValue: any) => {
        ...
        // 源码修改: 针对PrimaryObjectControl配置, 调整属性路径为 [basePath].[targetPropertyName], 原来逻辑为 [basePath].[所有values的panelIdPropertyName对应的属性值]
        const propertyPath = `${basePath}.${isPrimaryObject ? config.targetPropertyName : widgetPanelPropertyValue[panelIdPropertyName]}`;
