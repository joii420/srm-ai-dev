import { createReducer } from "utils/ReducerUtils";
import type { ReduxAction } from "@appsmith/constants/ReduxActionConstants";
import {
  ReduxActionTypes,
  ReduxActionErrorTypes,
} from "@appsmith/constants/ReduxActionConstants";

const initialState: AppViewReduxState = {
  isFetchingPage: false,
  initialized: false,
  headerHeight: 0,
  allLoading: false,
  globalLoading: false,
  isInitAfterExecuteWidgetFns: false,
};

const appViewReducer = createReducer(initialState, {
  [ReduxActionTypes.RESET_EDITOR_SUCCESS]: (state: AppViewReduxState) => {
    return { ...state, initialized: false };
  },
  [ReduxActionTypes.INITIALIZE_PAGE_VIEWER]: (state: AppViewReduxState) => {
    return { ...state, initialized: false };
  },
  [ReduxActionTypes.INITIALIZE_PAGE_VIEWER_SUCCESS]: (
    state: AppViewReduxState,
  ) => {
    return { ...state, initialized: true };
  },
  [ReduxActionTypes.ALL_COMPLETE_LOADING]: (state: AppViewReduxState) => {
    return { ...state, allLoading: true };
  },
  [ReduxActionTypes.ALL_COMPLETE_LOADING_SUCCESS]: (
    state: AppViewReduxState,
  ) => {
    return { ...state, allLoading: false };
  },
  [ReduxActionTypes.START_GLOBAL_LOADING]: (state: AppViewReduxState) => {
    return { ...state, globalLoading: true };
  },
  [ReduxActionTypes.END_GLOBAL_LOADING]: (state: AppViewReduxState) => {
    return { ...state, globalLoading: false };
  },
  [ReduxActionTypes.END_INIT_EXEC_WIDGET_FNS]: (state: AppViewReduxState) => {
    return { ...state, isInitAfterExecuteWidgetFns: true };
  },
  [ReduxActionTypes.FETCH_PUBLISHED_PAGE_INIT]: (state: AppViewReduxState) => {
    return { ...state, isFetchingPage: true };
  },
  [ReduxActionErrorTypes.FETCH_PUBLISHED_PAGE_ERROR]: (
    state: AppViewReduxState,
  ) => {
    return { ...state, isFetchingPage: false };
  },
  [ReduxActionTypes.FETCH_PUBLISHED_PAGE_SUCCESS]: (
    state: AppViewReduxState,
  ) => {
    return {
      ...state,
      isFetchingPage: false,
    };
  },
  [ReduxActionTypes.SET_APP_VIEWER_HEADER_HEIGHT]: (
    state: AppViewReduxState,
    action: ReduxAction<number>,
  ) => {
    return {
      ...state,
      headerHeight: action.payload,
    };
  },
});

export interface AppViewReduxState {
  initialized: boolean;
  isFetchingPage: boolean;
  headerHeight: number;
  allLoading: boolean;
  globalLoading: boolean;
  isInitAfterExecuteWidgetFns: boolean;
}

export default appViewReducer;
