...
export default function* executePluginActionTriggerSaga(
  ...
  const pluginActionNameToDisplay = getPluginActionNameToDisplay(action, state);
  ...
  if (isError) {
    ...
    if (onError) {
      ...
      const errorMsg: Record<string, any> = {
        title: createMessage(ERROR_PLUGIN_ACTION_EXECUTE, action.name),
        actionCode: payload.statusCode,
        message: payload.body,
      };
      throw new PluginTriggerFailureError(JSON.stringify(errorMsg), []);
      // throw new PluginTriggerFailureError(
...
export function* runActionSaga(
  ...
  const state: AppState = yield select((state: AppState) => state);
  const pluginActionNameToDisplay = getPluginActionNameToDisplay(actionObject, state);
...
function* executePageLoadActionsSaga(
  ...
  try {
    ...
    // yield put({
    //   type: "ALL_COMPLETE_LOADING_SUCCESS",
    // });
    yield put({
      type: ReduxActionTypes.UPDATE_IS_INIT_END,
      payload: { isInitEnd: { isEnd: true } },
    });
...
function* executePluginActionSaga(
  ...
  const state: AppState = yield select((state: AppState) => state);
  const pluginActionNameToDisplay = getPluginActionNameToDisplay(pluginAction, state);
  parentSpan &&
  ...
  const executeActionRequest: ExecuteActionRequest = {
    actionId: actionId,
    viewMode: pluginAction.moduleInstanceId
      ? false
