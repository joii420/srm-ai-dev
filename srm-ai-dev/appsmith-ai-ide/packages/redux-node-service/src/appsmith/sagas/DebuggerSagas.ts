...
function* onEntityDeleteSaga(payload: Log[]) {
  ...
  const moduleActions: any[] = yield select(getAllModuleJSCollections);

  const logSourceIds: string[] = [];
  for (let i = 0, len = sortedLogs.withSource.length; i < len; i++) {
    const log = sortedLogs.withSource[i];
    if (log.source?.id) {
      if (log.source.type === ENTITY_TYPE.MODULE_INSTANCE) {
        moduleActions.forEach(item => {
          if (item.config.moduleInstanceId === log.source!.id) {
            logSourceIds.push(item.config.id);
          }
        })
      } else {
        logSourceIds.push(log.source.id);
      }
    }
  }
