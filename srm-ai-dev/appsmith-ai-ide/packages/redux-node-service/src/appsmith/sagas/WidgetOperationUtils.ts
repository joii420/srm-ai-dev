...
import {
  getFocusedWidget,
  getSelectedWidget,
  getSFTabSelectedTabWidgetId,
  getWidgetMetaProps,
  getWidgets,
} from "./selectors";
import { getIsAnvilLayout } from "layoutSystems/anvil/integrations/selectors";
import { queryPageSelector } from "utils/hooks/useDomHelper";
...
export const getParentWidgetIdForPasting = function* (
  ...
  if (
    ...
    if (parentWidget.type !== "TABS_WIDGET") {
      ...
      //the child maybe is like container widget,such as custom sftab dashboard and so on
      if (
        parentWidget.type === "SFTAB_WIDGET" ||
        parentWidget.type === "SFDASHBOARD_WIDGET"
      ) {
        const selectedTabWidgetId: string = yield select(
          getSFTabSelectedTabWidgetId,
          parentWidget,
        );
        ...
        childWidget =
          parentWidget.children && widgets[parentWidget.children[0]];
      }
    } else {
...
function getStickyCanvasDOM(canvasId: string, ps?: string) {
  // get DOM of the overall canvas including it's total scroll height
  const stickyCanvasDOM = queryPageSelector(
...
export function getMousePositions(
  ...
  const stickyCanvasDOM = getStickyCanvasDOM(canvasId, ps);
 ...
 * @returns
 */
export function getDefaultCanvas(canvasWidgets: CanvasWidgetsReduxState, ps?: string) {
  const containerDOM = queryPageSelector(".t--modal-widget", ps);
  ...
  if (containerDOM && containerDOM.id && canvasWidgets[containerDOM.id]) {
    ...
    const { canvasDOM, canvasId } = getCanvasIdForContainer(containerWidget, undefined, ps);
    return {
      canvasId,
      canvasDOM,
      containerWidget,
 ...
 * @returns
 */
export function getContainerIdForCanvas(canvasId: string, ps?: string): string | undefined {
  if (canvasId === MAIN_CONTAINER_WIDGET_ID) return canvasId;

  const selector = `#${getStickyCanvasName(canvasId)}`;
  const canvasDOM = queryPageSelector(selector, ps);
 ...
 * @returns
 */
export function getCanvasIdForContainer(
  layoutWidget: WidgetProps,
  id?: string,
  ps?: string,
) {
  const selector =
  ...
  const containerDOM = queryPageSelector(selector, ps);
  if (!containerDOM) return {};
  const dropTargetDOM = containerDOM.querySelector(".t--drop-target");

  //get target canvas by selected widgetid if widget is custom like container widget,such as sftab dashboard and so on
  if (
    id &&
    (layoutWidget.type === "SFTAB_WIDGET" ||
      layoutWidget.type === "SFDASHBOARD_WIDGET")
  ) {
    const canvasDOM = containerDOM.querySelector(
      `canvas#canvas-selection-${id}`,
    );
    const dropTargetDOMs = containerDOM.querySelectorAll(".t--drop-target");
    const dropTargetDOM = [...dropTargetDOMs].find((i) =>
      i.contains(canvasDOM),
    );
    return {
      canvasId: canvasDOM ? canvasDOM.id.split("-")[2] : undefined,
...
export const groupWidgetsIntoContainer = function* (
  ...
  const parentColumnSpace =
    getParentColumnSpace(canvasWidgets, pastingIntoWidgetId, ps) ||
...
export function getParentColumnSpace(
  ...
  const containerId = getContainerIdForCanvas(pastingIntoWidgetId, ps);

  if (containerId === undefined) return;

  const containerWidget = canvasWidgets[containerId];
  const canvasDOM = queryPageSelector(
