...
import { showDrawer, showModal } from "actions/widgetActions";
import type {
...
function* openOrCloseModalSaga(action: ReduxAction<{ widgetIds: string[] }>) {
  ...
  let modalOrDrawerWidgetToOpen: string = action.payload.widgetIds[0];
  ...
  const widgetIsModal =
    ...
    modalWidgetIds.includes(modalOrDrawerWidgetToOpen);

  const widgetIsDrawer =
    // Check if the widget is a modal widget
    drawerWidgetIds.includes(modalOrDrawerWidgetToOpen);

  // Let's assume that this is not a child of a modal widget
  let widgetIsChildOfModal = false;
  let widgetIsChildOfDrawer = false;

  if (!widgetIsModal && !widgetIsDrawer) {
    ...
    if (indexOfParentModalWidget > -1) {
      ...
      modalOrDrawerWidgetToOpen = widgetAncestry[indexOfParentModalWidget];
    }

    // Check if the widget is a child of a modal widget
    const indexOfParentDrawerWidget: number = widgetAncestry.findIndex((id) =>
      drawerWidgetIds.includes(id),
    );
    // If we found a modal widget in the ancestry, we want to open that modal
    if (indexOfParentDrawerWidget > -1) {
      // Set the flag to true, so that we can open the modal
      widgetIsChildOfDrawer = true;
      modalOrDrawerWidgetToOpen = widgetAncestry[indexOfParentDrawerWidget];
    }
  }
  const isAnvilLayout: boolean = yield select(getIsAnvilLayout);
  if (isAnvilLayout) {
    // If widget is modal and modal is already open, skip opening it
    const modalProps = allWidgets[modalOrDrawerWidgetToOpen];
    ...
    if (
      (widgetIsModal || widgetIsChildOfModal || widgetIsChildOfDrawer) &&
  ...
  if (widgetIsModal || widgetIsChildOfModal) {
    yield put(showModal(modalOrDrawerWidgetToOpen));
  }
  ...
  if (widgetIsDrawer || widgetIsChildOfDrawer) {
    yield put(showDrawer(modalOrDrawerWidgetToOpen));
  }
