...
export default class JSObjectCollection {
  ...
  static getVariableState(
    ...
    if (!JSObjectName || !this.variableState) {
      //修改
      let dataTemp = Object.assign({}, this.variableState, {
        INIT: {
          ...this.variableState?.INIT,
          g_pmsaa_m: {
            ...this.variableState?.INIT?.g_pmsaa_m,
            source: undefined,
          },
        },
      });
      let safeTree = klona(dataTemp);
      if (this.variableState?.INIT?.g_pmsaa_m?.source) {
        safeTree.INIT.g_pmsaa_m.source =
          this.variableState?.INIT?.g_pmsaa_m.source;
      }
      return safeTree;
    }
