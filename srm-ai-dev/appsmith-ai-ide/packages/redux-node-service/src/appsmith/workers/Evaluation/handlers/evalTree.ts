...
export function evalTree(request: EvalWorkerSyncRequest) {
  ...
  if (webworkerTelemetry?.["transferDataToWorkerThread"]) {
  ...
  try {
    if (webworkerTelemetry?.__spanAttributes) {
    ...
    if (!dataTreeEvaluator) {
      ...
      const dataTreeResponse = profileFn(
        ...
        { description: "during initialisation" },
        webworkerTelemetry,
        () => dataTreeEvaluator?.evalAndValidateFirstTree(dataObj),
      );
      ...
      if (dataObj && !dataObj.status) {
        let newData = {
          contextTree: dataObj.contextTree,
          setFirstData: dataObj.setFirstDataMap,
        };
        fetch(
    ...
    () => {
      ...
      if (isNewTree) {
        try {
          ...
          updates = [{ kind: "newTree", rhs: dataTree }];
