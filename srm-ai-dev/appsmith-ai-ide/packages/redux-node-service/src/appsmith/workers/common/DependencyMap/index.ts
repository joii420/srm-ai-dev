 ...
 * @param configTree
 */
export function createDependencyMapNodes(
  dataTreeEvalRef: DataTreeEvaluator,
  unEvalTree: DataTree,
  configTree: ConfigTree,
  // dependMap: DependencyMap,
) {
  // const dependencyMap = dependMap;
  const { allKeys, dependencyMap } = dataTreeEvalRef;
  const allAppsmithInternalFunctions = convertArrayToObject(
    AppsmithFunctionsWithFields,
  );
      ...
      switch (event) {
        case DataTreeDiffEvent.NEW: {
          ...
          if (isWidgetActionOrJsObject(entity)) {
            // debugger;
            //修改configTree 在正式环境中不走以下逻辑
            if (unEvalDataTree.appsmith.mode !== "PUBLISHED") {
              if (
